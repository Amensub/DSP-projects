...
void processaudio_setup(void) {
	...
	new_delay_len_samples = delay_len_samples;
...
//	}
	...
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		...
		audiochannel_0_left_out[i] = audiochannel_0_left_in[i] * delay_dry_mix + delayed_left * delay_wet_mix;
		audiochannel_0_right_out[i] = audiochannel_0_right_in[i] * delay_dry_mix + delayed_right * delay_wet_mix;

		delay_line_left[delay_line_index] = audiochannel_0_left_in[i] + delayed_left * delay_feedback;
		delay_line_right[delay_line_index] = audiochannel_0_right_in[i] + delayed_right * delay_feedback;

		delay_line_index = (delay_line_index + 1) % delay_len_samples;
...
void processaudio_background_loop(void) {
	...
	if(multicore_data -> audioproj_fin_sw_2_core1_pressed)
	{
		...
		{
			new_delay_len_samples = delta_samples;
		}
		last_press_cyclecounter = current_cyclecounter;

	}
	for(uint32_t i = delay_len_samples; i < new_delay_len_samples; i++)
			{
				delay_line_left[i] = 0;
			}
