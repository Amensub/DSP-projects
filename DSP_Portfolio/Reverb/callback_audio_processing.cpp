...
void processaudio_setup(void) {
	...
	comb[0].length = 901 * mult;
	comb[1].length = 778 * mult;
	comb[2].length = 1011 * mult;
	comb[3].length = 1123 * mult;
	apf[0].length = 125 * mult;
	apf[1].length = 42 * mult;
	apf[2].length = 12 * mult;
...
//	}
	...
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		...
		combo1 = comb[0].docomb(audiochannel_0_left_in[i]);
		combo2 = comb[1].docomb(audiochannel_0_left_in[i]);
		combo3 = comb[2].docomb(audiochannel_0_left_in[i]);
		combo4 = comb[3].docomb(audiochannel_0_left_in[i]);

		y = (combo1 + combo2 + combo3 + combo4)/4;

		y = apf[0].doallpass(y);
		y = apf[1].doallpass(y);
		y = apf[2].doallpass(y);

		audiochannel_0_left_out[i] = y;
