...
bool mode = false;
float depthVal = 150;
float fracDelay, frac, rate, depth, LFO, mix, outL, outR;
int intDelay;
...
//	}
	...
	rate = multicore_data->audioproj_fin_pot_hadc0*10;
	depth= multicore_data->audioproj_fin_pot_hadc1*depthVal;
	mix= multicore_data->audioproj_fin_pot_hadc2;

	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {

		audiochannel_0_left_in[i] += outL * fb;
		audiochannel_0_left_in[i] += outR * fb;

		buffer[0][0] = audiochannel_0_left_in[i];
		buffer[1][0] = audiochannel_0_right_in[i];

		LFO = depth/2 * sin(2*PI*rate*counter/AUDIO_SAMPLE_RATE) + depth;

		fracDelay = fmod(LFO, maxDelay);
		intDelay = floor(fracDelay);

		frac = fracDelay - intDelay;

		outL = (1-frac)*buffer[0][intDelay] + frac*buffer[0][(intDelay+1)%maxDelay];
		outR = (1-frac)*buffer[1][intDelay] + frac*buffer[1][(intDelay+1)%maxDelay];

		audiochannel_0_left_out[i] = outL*mix + audiochannel_0_left_in[i]*(1-mix);
		audiochannel_0_right_out[i] = outR*mix + audiochannel_0_right_in[i]*(1-mix);

		counter++;

		for (int j = maxDelay; j > 0; j--) {
