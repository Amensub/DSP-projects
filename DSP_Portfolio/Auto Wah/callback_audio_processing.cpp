...
float cutoff, Q, filterMod;
float TC = log(9.0);
float AT = exp(-TC/(0.05*48000));
float RT = exp(-TC/(0.2*48000));
float beta, env, env_prev, detector;
...
//
//	}
	filterMod = multicore_data->audioproj_fin_pot_hadc0*7000;

	cutoff = 20+detector*filterMod;
	Q = multicore_data->audioproj_fin_pot_hadc1*18 + 2;
	...
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		...
		float rect = (audiochannel_0_left_in[i])*(audiochannel_0_left_in[i]);
		if(rect > env_prev){
			beta = AT;
		}
		else{
		...
		env = (1-beta)*rect + (beta*env_prev);

		bandPassL.set(cutoff, Q);
		bandPassR.set(cutoff, Q);

		detector = sqrt(env);

		audiochannel_0_left_out[i] = bandPassL.doBiquad(audiochannel_0_left_in[i]);
		audiochannel_0_right_out[i] = bandPassR.doBiquad(audiochannel_0_right_in[i]);
