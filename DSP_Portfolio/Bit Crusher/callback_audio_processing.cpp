 ...
 */
int N = 16; // target quantization bits
float Q; // minimization quantization level
bool treadRiser;
float outL, outR;

int downSampleFactor, count;
...
//	}
	...
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		...
		Q = 2 / powf(2.0, N);
		float iL = audiochannel_0_left_in[i];
		float iR = audiochannel_0_right_in[i];

		if(treadRiser == true)
		{
			outL = (floor(iL/Q) + 0.5) * Q;
			outR = (floor(iR/Q) + 0.5) * Q;
		}
		else
		{
			outL = floor(iL/Q + 0.5) * Q;
		...
		audiochannel_0_left_out[i] = currentSampleL;
		audiochannel_0_right_out[i] = currentSampleR;
		downSampleFactor = ceil(multicore_data->audioproj_fin_pot_hadc1 * 48 + 0.001);
		count++;
