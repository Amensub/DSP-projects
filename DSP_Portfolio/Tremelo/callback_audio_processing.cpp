...
//
//	}
	depth = multicore_data->audioproj_fin_pot_hadc0;
	phi = multicore_data->audioproj_fin_pot_hadc1 * PI;
	...
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		...
		A_L = (depth/2) * sin(2*PI*(rate/AUDIO_SAMPLE_RATE)*counter) + (1 - depth/2);
		A_R = (depth/2) * sin(2*PI*(rate/AUDIO_SAMPLE_RATE)*counter + phi) + (1 - depth/2);
		...
		if(bypass == true){
			...
			audiochannel_0_left_out[i] = audiochannel_0_left_in[i] * A_L;
			audiochannel_0_right_out[i] = audiochannel_0_right_in[i] * A_R;
		}
...
void processaudio_background_loop(void) {
	...
	if(multicore_data->audioproj_fin_sw_2_core1_pressed){
		...
		if(delta_seconds < 5.0 && delta_seconds > 0.05)
			rate = 2*(1/delta_seconds);
