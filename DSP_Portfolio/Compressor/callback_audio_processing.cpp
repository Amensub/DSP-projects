 ...
 */
float AT, RT, Threshold, rect, alpha, env_current, d, g, attk, relt;
float TC = log(9.0);
float env_prev = 0;
float Ratio[5] = {1.0, 2.0, 4.0, 9.0, 20.0};
int RatioN = 0;
...
//	}
	...
	attk = multicore_data->audioproj_fin_pot_hadc0*0.1 + 0.005; // Attack Time
	relt = multicore_data->audioproj_fin_pot_hadc1*0.1 + 0.005; // Release Time
	AT = exp(-TC/(attk * AUDIO_SAMPLE_RATE));
	RT = exp(-TC/(relt * AUDIO_SAMPLE_RATE));
	Threshold = multicore_data->audioproj_fin_pot_hadc2 * -60; // from 0 to -20 dB
	...
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		...
		rect = audiochannel_0_left_in[i] * audiochannel_0_left_in[i];
		...
		env_current = ((1 - alpha) * rect) + (alpha * env_prev);

		env_prev = env_current;

		d = 20*log10(sqrt(env_current));
		...
		{
			g = Threshold + (d - Threshold)/Ratio[RatioN] - d;
		}

		audiochannel_0_left_out[i] = (audiochannel_0_left_in[i]) * (pow(10, g / 10));
