...
//	}
	...
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		...
		if(type == 0)
		{
			for (int j = 0; j < 8; j++)
			{
				tempAudio += sin(2.0*PI*keys[ j ].freq*counter/AUDIO_SAMPLE_RATE);
			}
		}
		...
		if(type == 1)
		{
			for (int j = 0; j < 8; j++)
			{
				...
				{
					tempAudio += (4/PI)*(1.0/((2.0*k)+1.0))*sin(2*PI*keys[ j ].freq*((2.0*k)+1.0)*counter/AUDIO_SAMPLE_RATE);
		...
		if(type == 2)
		{
			for (int j = 0; j < 8; j++)
			{
				for(int k = 0; k < numPartials; k++)
				{
					tempAudio += (-2.0/PI)*(1.0/(k+1.0))*sin(2.0*PI*keys[ j ].freq*(k+1.0)*counter/AUDIO_SAMPLE_RATE);
		...
		if(type == 3)
		{
			for (int j = 0; j < 8; j++)
			{
				for(int k = 0; k < numPartials; k++)
				{
					float temp = (1.0/((2.0*k+1.0)*(2.0*k+1.0)));
					tempAudio += (8.0/pi_2)*temp*sin(2*PI*keys[ j ].freq*((2.0*k)+1.0)*counter/AUDIO_SAMPLE_RATE);
				}
			}
		}
